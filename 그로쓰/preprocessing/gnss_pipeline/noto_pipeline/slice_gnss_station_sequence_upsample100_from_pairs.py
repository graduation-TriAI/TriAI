...
from shared.paths import GNSS_NOTO_PROC, GNSS_NOTO_CSV, PAIRS_NOTO_CSV
from shared.config import WIN, STRIDE

# =============
# Config
# =============
PAIRS_CSV = PAIRS_NOTO_CSV / "noto_station_pairs_ver_30km.csv"
...
base_dir = GNSS_NOTO_PROC / f"{WIN}_{STRIDE}" / "100hz"
base_dir.mkdir(parents=True, exist_ok=True)

OUT_NPZ = base_dir / (
    f"noto_gnss_station_seq_{WIN_SEC}s_{STRIDE_SEC}s_{TARGET_FS}hz_from_pairs30km.npz"
)
...
def get_station_id(fp: Path):
    return fp.stem.split("_")[-2].upper()

def normalize_station(series: pd.Series) -> pd.Series:
    return series.astype(str).str.strip().str.upper()

pairs_df = pd.read_csv(PAIRS_CSV)
print("before dropna:", len(pairs_df))
...
print("after dedup:", len(pairs_df))
...
for fp in sorted(CSV_DIR.glob("*.csv")):
    gnss_station = get_station_id(fp)
    ...
    try:
        ...
        failed_read += 1
        print("Failed to read file:", fp.name, "->", type(e).__name__, e)
        continue
